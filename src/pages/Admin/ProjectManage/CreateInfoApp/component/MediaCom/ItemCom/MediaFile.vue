<template>
  <div>
    <div class="tips" v-if="mediaType === ''">请先选择资源类型22</div>
    <div v-else>
      <CustomUploadZip
        ref="uploadRef"
        :btnText="btnText"
  ...
  watch: {
    ...
    baseData: {
      ...
      handler() {
        ...
        this.uploadInfo = {
          // 修改
          folder: `${basePicData.userCode}/${basePicData.projectCode}/${basePicData.appTypeName}/`,
          fileNum: ''
        }
      }
    }
    ...
    getMediType(fileName) {
      ...
      if (baseItem) {
        if (mtlType === 'mtl' && type === 'txt') {
          ...
          let value =
            mediaTypeConfig.find(
              (v) => v.reg.test(type) && v.mediaType === this.mediaType
            ) || mediaTypeConfig.find((v) => v.reg.test(type))
          if (value) {
            // result = value.value
