  ...
  <div class="mediaItem">
    <div class="mediaBox">
      <div class="mediaContorl">
        ...
        <div
          :class="['controlIcon', openCode === data.mediaType ? 'open' : '']"
          @click="handleTrigger"
        >
      ...
      <div class="mediaInfo">
        <el-form label-width="120px" class="mediaInfoMain">
          <el-form-item label="资源类型:">
            <MediaType @preview="handleTriggerPhone2" v-model="mediaType" />
          </el-form-item>
...
export default {
  ...
  watch: {
    ...
    paramInfo() {
      console.log(this.paramInfo, 'paramInfoparamInfo')
  ...
  data() {
    return {
      ...
      highvalue: true,
      valueText: '开启',
      paramInfo: this.paramInfoList[this.index] || {
        position: '0|0|0',
        rotate: '0|0|0',
        scale: '1|1|1',
        width: 0,
  ...
  methods: {
    ...
    changehighvalue() {
      ...
      console.log(this.highvalue, 'changehighvalue')
    ...
    async getParamFun() {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        console.log('mediaCode:-----2', this.paramInfoList, data)

        result.push({
          projectCode: projectCode,
          // mediaCode: this.mediaDataList().mediaCode || this.data.mediaCode,
          mediaCode: this.data.mediaCode,
          modelParamCode: this.paramInfoList.modelParamCode || data,
          ...
          mediaFun: async () => {
            ...
            for (let i = 0; i < list.length; i++) {
              ...
              result.push({
                ...
                // mediaCode: this.mediaDataList().mediaCode || data
                mediaCode: this.data.mediaCode
              })
            }
            return result
    ...
    handleTriggerPhone2() {
      ...
      this.$store.commit('SET_PARAMINDEX', this.index)
    },
    handleDelete() {
      this.$emit('delete', {
        data: this.data,
    ...
    triggerSlide() {
      console.log('this.openCode', this.data.mediaCode, this.openCode)
      // console.log("this.mediaDataList",this.mediaDataList()[1].mediaType)
    ...
    // 切换类型 重置所有资源信息
    reset() {
      // await this.paramsList(this.mediaType)
      // this.paramInfo = this.paramInfoList()
      // this.mediaType = this.mediaDataList()[1].MediaType
      // this.baseParamList = this.mediaDataList()
      // this.baseParamList = this.$options.data().baseParamList
      // console.log('测试111', this.baseParamList)
      // this.baseParmCodeToSnowCodeInfo = {}
      this.$refs.mediaFileRef.reset()
    },
    handleTrigger() {
      if (
        this.openCode !== this.mediaDataList.mediaCode ||
        this.data.mediaCode
      ) {
        this.$emit(
          'update:openCode',
          this.mediaDataList.mediaCode || this.data.mediaCode
        )
      }
    }
  },
  created() {
    ...
    // console.log(this.paramInfoList, 'this.index')
    // console.log(this.paramInfoList[this.index], 'this.index')
    // let paramList = this.paramInfoList[this.index] || {
    //   position: '0|0|0',
    //   rotate: '0|0|0',
    //   scale: '1|1|1',
    //   width: 0,
