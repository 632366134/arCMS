...
export default {
      ...
      configList: [
        {
          mediaType: 3,
          text: '上传压缩包',
          tips: `请将图片压缩成zip(压缩文件内部名称不能包含中文汉字，标点符号)。图片仅支持${MediaLimit.mediaPicType}文件，尺寸不超过${MediaLimit.mediaPicPX}像素,文件大小不超过${MediaLimit.mediaPicSize}M`,
          accept: MediaLimit.mediaPicType
        },
        {
          mediaType: 4,
          text: '上传压缩包',
          tips: `请将视频和封面图压缩成zip(压缩文件内部名称不能包含中文汉字，标点符号)。视频仅支持${MediaLimit.mediaVideoType}文件并且大小不超过${MediaLimit.mediaVideoSize}M,封面图尺寸不超过${MediaLimit.mediaVideoPicPX}`,
          accept: '.zip/.rar'
        },
        {
          mediaType: 5,
          text: '上传压缩包',
          tips: `请将模型、贴图等压缩成zip(压缩文件内部名称不能包含中文汉字，标点符号)。模型仅支持${MediaLimit.mediaModelType}文件并且大小不超过${MediaLimit.mediaModelSize}M, 贴图尺寸不超过${MediaLimit.mediaModelPicPX}，大小不超过${MediaLimit.mediaModelPicSize}M`,
          accept: '.zip/.rar'
        },
        {
          mediaType: 6,
        ...
        },
        {
          mediaType: 12,
          text: '上传压缩包',
          tips: `请将模型、贴图等压缩成zip(压缩文件内部名称仅支持数字和英文)。模型仅支持${MediaLimit.mediaModelType}文件并且大小不超过${MediaLimit.mediaModelSize}M, 贴图尺寸不超过${MediaLimit.mediaModelPicPX}，大小不超过${MediaLimit.mediaModelPicSize}M`,
          accept: '.zip/.rar'
        }
      ],
      fileList: [],
  ...
  methods: {
    ...
    getMediType(fileName = this.data.mediaObsName) {
        ...
        data: () => {
          return this.realFileList.map((item) => {
            return {
              ...
              mediaObsName: item.fileOssName || this.data.mediaObsName,
      ...
      return this.check().then((res) => {
        ...
        if (res.code === 1000) {
          return this.$refs.uploadRef
            ...
            .then((res) => {
              console.log(res, this.indexFun(), '12312333')
              res.successList[0].mediaCode = this.mediaCode
              this.$store.commit('SET_UPLOADINDEX', this.indexFun())
              this.$store.commit('SET_successUploadList', res.successList[0])
            })
        }
      })
    },
    ...
    reset() {
      // this.fileList = this.realFileList = [
      //     {
      //       name: this.mediaDataList()[1].mediaObsName,
      //       url: this.mediaDataList()[1].mediaObsPath
      //     }
      //   ]
      this.fileList = []
      this.realFileList = []
  ...
  created() {
    ...
    // this.resetIndex = 'resetMediaFile' + this.$getCode()
    // this.registeredResetEvent(this.resetIndex, this.reset)
    console.log(this.data, '查看压缩包相关信息')
    this.fileList = this.realFileList = [
      {
        name: this.data.mediaObsName,
        url: this.data.mediaObsPath
      }
    ]
  },
  beforeDestroy() {
