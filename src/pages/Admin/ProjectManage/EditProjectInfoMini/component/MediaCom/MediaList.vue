<template>
  ...
  <div class="mediaList">
    <div>
      ...
      <!-- <el-button @click="handleAdd" v-d type="primary" :disabled="true">
        修改小程序端资源
      </el-button> -->
      <!-- <el-button @click="handleAdd" v-d type="primary" >
        修改小程序端资源
      </el-button> -->
...
export default {
  ...
  methods: {
    async getParam(index) {
      console.log('getParam', this.mediaList, index)
      let modelParamList = []
    ...
    async getData() {
      ...
      for (let i = 0; i < refs.length; i++) {
        ...
        paramFunList.push(mediaData.modelParamFun) // 模型参数设置
      }
      ...
      return {
        ...
        fun: async () => {
          ...
          console.log(insertMediaList, 'insertMediaList----')
          return {
    ...
    handleItemDelete({ data, index }) {
      ...
      this.$confirm('您确定要删除这个资源吗?', '提示')
        .then(() => {
          ...
          console.log(index, '确定要删除这确定要删除这')
          this.mediaList.splice(index, 1)
          // this.$store.commit('Del_successUploadList', index)
          this.$store.commit('SET_PARAMINDEX', index)
  ...
  created() {
    let mediaDataList = this.mediaDataListFun()

    let list = mediaDataList
    this.mediaList = list.slice(1) ?? []
    ...
    console.log(this.mediaList, 'medialist1231213123')
  },
  beforeDestroy() {
